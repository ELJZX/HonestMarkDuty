from django.urls import path

from inventory import views

app_name = "inventory"

urlpatterns = [
    path("", views.InventoryBoardView.as_view(), name="board"),
    path("list/", views.InventoryItemListView.as_view(), name="item_list"),
    path("storage/<int:pk>/", views.InventoryStorageTypesView.as_view(), name="storage_types"),
    path("storages/", views.StorageListView.as_view(), name="storage_list"),
    path("storages/new/", views.StorageCreateView.as_view(), name="storage_create"),
    path("storages/<int:pk>/edit/", views.StorageUpdateView.as_view(), name="storage_update"),
    path("storages/<int:pk>/delete/", views.StorageDeleteView.as_view(), name="storage_delete"),
    path("new/", views.InventoryItemCreateView.as_view(), name="item_create"),
    path("<int:pk>/", views.InventoryItemDetailView.as_view(), name="item_detail"),
    path("<int:pk>/edit/", views.InventoryItemUpdateView.as_view(), name="item_update"),
    path("<int:pk>/delete/", views.InventoryItemDeleteView.as_view(), name="item_delete"),
    path("<int:pk>/movement/", views.InventoryMovementCreateView.as_view(), name="movement_create"),
    path("<int:pk>/kind/", views.InventoryItemTypeChangeView.as_view(), name="item_kind_change"),
    path("locations/", views.StorageLocationListView.as_view(), name="location_list"),
    path("locations/new/", views.StorageLocationCreateView.as_view(), name="location_create"),
    path("locations/<int:pk>/edit/", views.StorageLocationUpdateView.as_view(), name="location_update"),
    path("categories/", views.ItemCategoryListView.as_view(), name="category_list"),
    path("categories/new/", views.ItemCategoryCreateView.as_view(), name="category_create"),
    path("categories/<int:pk>/edit/", views.ItemCategoryUpdateView.as_view(), name="category_update"),
    path("types/", views.ItemTypeListView.as_view(), name="kind_list"),
    path("types/new/", views.ItemTypeCreateView.as_view(), name="kind_create"),
    path("types/<int:pk>/edit/", views.ItemTypeUpdateView.as_view(), name="kind_update"),
    path("types/<int:pk>/delete/", views.ItemTypeDeleteView.as_view(), name="kind_delete"),
]
