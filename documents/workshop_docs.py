...
SIGNATURE_LINE_RE = re.compile(r"^(?P<before>.*?)(?P<us>(?:_{3,}\s*)+)(?P<after>.*)$")
QUOTE_PAIRS = (("«", "»"), ("“", "”"), ('"', '"'))

WORKSHOP_DOCUMENTS = [
    {"name": "Цех №1", "code": "ceh1"},
    {"name": "Кисломолочный цех", "code": "kmc"},
    {"name": "Цельномолочный цех", "code": "cmc"},
...
def _replace_quoted(text: str, token: str, value: str) -> str:
    """Заменяет токен в кавычках («ФИО», "ФИО", “ФИО”) на значение."""
    if not value:
        return text
    for left, right in QUOTE_PAIRS:
        text = text.replace(f"{left}{token}{right}", value)
    return text


def _render_signature_line(text: str, position: str, specialist: str) -> str:
    """Подпись: должность слева, ``_______ ФИО`` — по правому краю (табуляция)."""
    match = SIGNATURE_LINE_RE.match(text)
    if not match:
        return text
    before = match.group("before")
    underscores = match.group("us").strip()
    after = match.group("after").strip()
    left = before.rstrip()
    if position and before.strip():
        left = position
    if after and specialist:
        right = f"{underscores} {specialist}"
    elif after:
        right = f"{underscores} {after}"
    else:
        right = underscores
    if left:
        return f"{left}\t{right}"
...
def render_sample_text(text: str, context: dict) -> str:
    ...
    if specialist:
        rendered = _replace_quoted(rendered, "ФИО", specialist)
    ...
    if position:
        rendered = _replace_quoted(rendered, "ДОЛЖНОСТЬ", position)
        rendered = rendered.replace("ДОЛЖНОСТЬ", position)
...
def _replace_in_paragraph(paragraph, context: dict, state: dict | None = None) -> None:
    ...
    is_signature = bool(SIGNATURE_LINE_RE.match(original))
    rendered = render_sample_text(original, context)

    if is_signature:
        paragraph.alignment = WD_ALIGN_PARAGRAPH.LEFT
        tab_position = context.get("signature_tab")
...
def _fill_sample(path: Path, context: dict) -> BytesIO:
    ...
    if (
        ...
        local_context["signature_tab"] = (
            section.page_width - section.left_margin - section.right_margin
        )

    _replace_in_container(docx, local_context)
    for part in docx.sections:
        _replace_in_container(part.header, local_context)
        _replace_in_container(part.footer, local_context)
